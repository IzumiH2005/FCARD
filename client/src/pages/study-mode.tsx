...
export default function StudyMode() {
  ...
  const { data: flashcards = [], isLoading } = useQuery({
    queryKey: ["/api/flashcards", studyId],
    queryFn: async () => {
      if (isBookStudy) {
        // Get all sections for the book, then get all flashcards
        const sectionsResponse = await apiRequest("GET", `/api/sections/${studyId}`);
        const sections = await sectionsResponse.json();
        
        const allFlashcards: FlashcardType[] = [];
        for (const section of sections) {
          const flashcardsResponse = await apiRequest("GET", `/api/flashcards/${section.id}`);
          const sectionFlashcards = await flashcardsResponse.json();
        ...
        const response = await apiRequest("GET", `/api/flashcards/${studyId}`);
