...
export default function BookDetail() {
  ...
  const exportBookMutation = useMutation({
    mutationFn: async () => {
      const response = await apiRequest("GET", `/api/books/export/${bookId}`);

      // Check if we're in a mobile app environment
      const isMobileApp = navigator.userAgent.includes('wv') || navigator.userAgent.includes('Median') || window.location.href.includes('median');

      if (isMobileApp) {
        // For mobile apps, try to use the download attribute or open in new tab
        ...
        try {
          ...
          setTimeout(() => {
            if (confirm('Si le téléchargement ne démarre pas, voulez-vous ouvrir le fichier dans un nouvel onglet ?')) {
          ...
          <>
            {sections.map((section) => (
              ...
              >
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div className="flex-1 cursor-pointer" onClick={() => handleSectionClick(section)}>
